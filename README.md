...
1) *Состояние гонки(race condition)* - возникает тогда, когда несколько потоков многопоточного приложения пытаются одновременно получить доступ к данным, причем хотя бы один поток выполняет запись, последствия могут быть не предсказуемыми.

Решение: использовать семафоры или группы отправки.

2) *Инверсия приоритетов* - по сути это то когда высокоприоритетный процесс ждет сигнала от низкоприоритетного процесса о том что он выполнил задачу.

Решение: принудительное повышение приоритетов

3) *Взаимная блокировка* - это когда несколько потоков находятся в ожидании доступа к ресурсам и не один из них не может начать выполнение пока не освободится доступ к ресурсу.
...
1) *Массивы* - это структура данных в которой значения упорядочненны в определенном порядке(по индексам).

2) *Множества* - это неупорядочненная структура данных в которой хранятся уникальные значения (подписана под протокл хэширования).

3) *Словари* - это неупорядочненная структура данных в которой значения хранятся в виде ключ-значение (подписана под протокл хэширования).
...
1) Использовать конструкцию guard let, if let.

2) Использовать опциональную цепочку.

3) Проверить через if(object != nil)
